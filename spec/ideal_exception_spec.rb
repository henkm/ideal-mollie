...
describe Mollie::IdealException do
  context "#new_order" do
    it "should throw exception when no config is set" do
      VCR.use_cassette("new_order", :match_requests_on => [:ignore_query_param_ordering]) do
        ...
        config.reset!
        expect { Mollie.new_order(1234, "exception test", "0031") }.
          to raise_error(Mollie::IdealException, /A fetch was issued without specification of 'partnerid'./)
      end
    end
    ...
    it "should throw an exception when a unknown account is specified" do
      ...
      VCR.use_cassette("new_order", :match_requests_on => [:ignore_query_param_ordering]) do
        expect { Mollie.new_order(1234, "exception test", "0031") }.
          to raise_error(Mollie::IdealException, /This account does not exist or is suspended./)
      end
    end
    ...
    it "should throw an exception when a unknown profile key is specified" do
      ...
      VCR.use_cassette("new_order", :match_requests_on => [:ignore_query_param_ordering]) do
        expect { Mollie.new_order(1234, "exception test", "0031") }.
          to raise_error(Mollie::IdealException, /A fetch was issued for an unknown or inactive profile./)
      end
    end
  end
  ...
  context "#check_order" do
    it "should raise an exception when a invalid transaction_id is asked" do
      ...
      VCR.use_cassette("check_order", :match_requests_on => [:ignore_query_param_ordering]) do
        expect { Mollie.check_order("482d599bbcc7795727650330ad65fe9b") }.
          to raise_error(Mollie::IdealException, /This is an unknown order./)
      end
    end
  end
end
